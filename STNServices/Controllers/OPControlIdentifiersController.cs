...
{
    [Route("[controller]")]
    public class OPControlIdentifiersController : STNControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            {
                //sm(agent.Messages);
                return Ok(agent.Select<op_control_identifier>());
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        public async Task<IActionResult> Get(int id)
        {
            try
            {
                ...
                //sm(agent.Messages);
                return Ok(await agent.Find<op_control_identifier>(id));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        [HttpGet("/ObjectivePoints/{objectivePointId}/OPControls")]
        public async Task<IActionResult> GetOPControls(int objectivePointId)
        {
            try
            {
                if (objectivePointId < 0) return new BadRequestResult();

                var objectRequested = agent.Select<op_control_identifier>().Where(x => x.objective_point_id == objectivePointId);
        ...
        public async Task<IActionResult> Post([FromBody]op_control_identifier entity)
        {
            try
            {
                ...
                //sm(agent.Messages);
                return Ok(await agent.Add<op_control_identifier>(entity));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        [Route("Batch")]
        public async Task<IActionResult> Batch([FromBody]List<op_control_identifier> entities)
        {
            try
            {
                ...
                //sm(agent.Messages);
                return Ok(await agent.Add<op_control_identifier>(entities));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var entity = await agent.Find<op_control_identifier>(id);
                if (entity == null) return new NotFoundResult();

                await agent.Delete<op_control_identifier>(entity);
                //sm(agent.Messages);
                return Ok();
            }
            catch (Exception ex)
