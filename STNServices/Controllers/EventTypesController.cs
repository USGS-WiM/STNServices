...
{
    [Route("[controller]")]
    public class EventTypesController : STNControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            {
                //sm(agent.Messages);
                return Ok(agent.Select<event_type>());
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        public async Task<IActionResult> Get(int id)
        {
            try
            {
                ...
                //sm(agent.Messages);
                return Ok(await agent.Find<event_type>(id));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        [HttpGet("/Events/{eventId}/Type")]
        public async Task<IActionResult> GetEventType(int eventId)
        {
            try
            {
                if (eventId < 0) return new BadRequestResult(); 

                var objectRequested = agent.Select<events>().Include(m => m.event_type).FirstOrDefault(x => x.event_id == eventId).event_type;
        ...
        [HttpPost][Authorize(Policy = "AdminOnly")]
        [Route("Batch")]
        public async Task<IActionResult> Batch([FromBody]List<event_type> entities)
        {
            try
            {
                if (!isValid(entities)) return new BadRequestObjectResult("Object is invalid");
                //sm(agent.Messages);
                return Ok(await agent.Add<event_type>(entities));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var entity = await agent.Find<event_type>(id);
                if (entity == null) return new NotFoundResult();

                await agent.Delete<event_type>(entity);
                //sm(agent.Messages);
                return Ok();
            }
            catch (Exception ex)
