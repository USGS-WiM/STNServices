...
{
    [Route("[controller]")]
    public class StatusTypesController : STNControllerBase
    {
        ...
        public async Task<IActionResult> Get()
        {
            ...
            {
                //sm(agent.Messages);
                return Ok(agent.Select<status_type>());
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        public async Task<IActionResult> Get(int id)
        {
            try
            {
                ...
                //sm(agent.Messages);
                return Ok(await agent.Find<status_type>(id));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        [HttpGet("/InstrumentStatus/{instrumentStatusId}/Status")]
        public async Task<IActionResult> GetInstrumentStatusStatus(int instrumentStatusId)
        {
            try
            {
                if (instrumentStatusId < 0) return new BadRequestResult(); 

                var objectRequested = agent.Select<instrument_status>().Include(m => m.status_type).FirstOrDefault(x => x.instrument_status_id == instrumentStatusId).status_type;
        ...
        [HttpPost][Authorize(Policy = "AdminOnly")]
        [Route("Batch")]
        public async Task<IActionResult> Batch([FromBody]List<status_type> entities)
        {
            try
            {
                if (!isValid(entities)) return new BadRequestObjectResult("Object is invalid");
                //sm(agent.Messages);
                return Ok(await agent.Add<status_type>(entities));
            }
            catch (Exception ex)
            {
                //sm(agent.Messages);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                var entity = await agent.Find<status_type>(id);
                if (entity == null) return new NotFoundResult();

                await agent.Delete<status_type>(entity);
                //sm(agent.Messages);
                return Ok();
            }
            catch (Exception ex)
