...
{
    public class NetworkNamesTest
    {
        public NetworkNamesController controller { get; private set; }
        public NetworkNamesTest() {
            //Arrange
            controller = new NetworkNamesController(new InMemoryNetworkNamesAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<network_name>>(okResult.Value);
        ...
        public async Task Get()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<network_name>(okResult.Value);
        ...
        {
            //Arrange
            var entity = new network_name() { name = "TestPost" };
        ...
        public async Task Put()
        {
            ...
            var entity = Assert.IsType<network_name>(okgetResult.Value);
            

            var newEntity = new network_name();
    ...
    public class InMemoryNetworkNamesAgent : ISTNServicesAgent
    {
        private List<network_name> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryNetworkNamesAgent() {
           this.entityList = new List<network_name>()
           {
               new network_name() { network_name_id = 1, name= "Not Defined" },
               new network_name() { network_name_id = 2, name= "SWaTH"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(network_name))
        ...
        {
            if (typeof(T) == typeof(network_name))
                return Task.Run(()=> { return entityList.Find(i => i.network_name_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(network_name))
            {
                entityList.Add(item as network_name);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(network_name))
            {
                entityList.AddRange(items.Cast<network_name>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(network_name))
            {
                var index = this.entityList.FindIndex(x => x.network_name_id == pkId);
                (item as network_name).network_name_id = pkId;
                this.entityList[index] = item as network_name;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(network_name))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as network_name); });
            }
