...
{
    public class DeploymentTypesTest
    {
        public DeploymentTypesController controller { get; private set; }
        public DeploymentTypesTest() {
            //Arrange
            controller = new DeploymentTypesController(new InMemoryDeploymentTypesAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<deployment_type>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Wave Height", result.LastOrDefault().method);
        }
        ...
        public async Task Get()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<deployment_type>(okResult.Value);
            
            Assert.Equal("Water Level", result.method);
        }
        ...
        {
            //Arrange
            var entity = new deployment_type() { method = "Barometric Pressure" };
            ...
            Assert.Equal("Barometric Pressure", result.method);
        }
        ...
        public async Task Put()
        {
            ...
            var entity = Assert.IsType<deployment_type>(okgetResult.Value);
            

            var newEntity = new deployment_type();
            newEntity.method = "water level";
            //should test the equals Equatable for all these too
            var huh = entity.Equals(newEntity);

            entity.method = "editPriority";
            //Act
            ...
            Assert.Equal(entity.method, result.method);
        }
    ...
    public class InMemoryDeploymentTypesAgent : ISTNServicesAgent
    {
        private List<deployment_type> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryDeploymentTypesAgent() {
           this.entityList = new List<deployment_type>()
           {
               new deployment_type() { deployment_type_id = 1, method= "Water Level" },
               new deployment_type() { deployment_type_id = 2, method= "Wave Height"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(deployment_type))
        ...
        {
            if (typeof(T) == typeof(deployment_type))
                return Task.Run(()=> { return entityList.Find(i => i.deployment_type_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(deployment_type))
            {
                entityList.Add(item as deployment_type);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(deployment_type))
            {
                entityList.AddRange(items.Cast<deployment_type>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(deployment_type))
            {
                var index = this.entityList.FindIndex(x => x.deployment_type_id == pkId);
                (item as deployment_type).deployment_type_id = pkId;
                this.entityList[index] = item as deployment_type;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(deployment_type))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as deployment_type); });
            }
