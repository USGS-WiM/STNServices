...
namespace STNServices.XUnitTest
{
    ...
    {
        public EventTypesController controller { get; private set; }
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<event_type>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Hurricane", result.LastOrDefault().type);
        }
        ...
        public async Task Get()
        {
            ...
            Assert.Equal("Riverine Flood", result.type);
        }
        ...
        {
            //Arrange
            var entity = new event_type() { type = "TestPost" };
    ...
    public class InMemoryEventTypesAgent : ISTNServicesAgent
    {
        private List<event_type> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryEventTypesAgent() {
           this.entityList = new List<event_type>()
           {
               new event_type() { event_type_id = 1, type= "Riverine Flood" },
               new event_type() { event_type_id = 2, type= "Hurricane"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(event_type))
        ...
        {
            if (typeof(T) == typeof(event_type))
                return Task.Run(()=> { return entityList.Find(i => i.event_type_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(event_type))
            {
                entityList.Add(item as event_type);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(event_type))
            {
                entityList.AddRange(items.Cast<event_type>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(event_type))
            {
                var index = this.entityList.FindIndex(x => x.event_type_id == pkId);
                (item as event_type).event_type_id = pkId;
                this.entityList[index] = item as event_type;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(event_type))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as event_type); });
            }
