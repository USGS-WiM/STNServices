...
{
    public class SensorBrandsTest
    {
        public SensorBrandsController controller { get; private set; }
        public SensorBrandsTest() {
            //Arrange
            controller = new SensorBrandsController(new InMemorySensorBrandsAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<sensor_brand>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Hobo", result.LastOrDefault().brand_name);
        }
        ...
        public async Task Get()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<sensor_brand>(okResult.Value);
            
            Assert.Equal("RBR Solo", result.brand_name);
        }
        ...
        {
            //Arrange
            var entity = new sensor_brand() { brand_name = "TestPost" };
            ...
            Assert.Equal("TestPost", result.brand_name);
        }
        ...
        public async Task Put()
        {
            ...
            var entity = Assert.IsType<sensor_brand>(okgetResult.Value);
            

            var newEntity = new sensor_brand();
            newEntity.brand_name = "rbr Solo";
            //should test the equals Equatable for all these too
            var huh = entity.Equals(newEntity);

            entity.brand_name = "testEdit";
            //Act
            ...
            Assert.Equal(entity.brand_name, result.brand_name);
        }
    ...
    public class InMemorySensorBrandsAgent : ISTNServicesAgent
    {
        private List<sensor_brand> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemorySensorBrandsAgent() {
           this.entityList = new List<sensor_brand>()
           {
               new sensor_brand() { sensor_brand_id = 1, brand_name= "RBR Solo" },
               new sensor_brand() { sensor_brand_id = 2, brand_name= "Hobo"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(sensor_brand))
        ...
        {
            if (typeof(T) == typeof(sensor_brand))
                return Task.Run(()=> { return entityList.Find(i => i.sensor_brand_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_brand))
            {
                entityList.Add(item as sensor_brand);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_brand))
            {
                entityList.AddRange(items.Cast<sensor_brand>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_brand))
            {
                var index = this.entityList.FindIndex(x => x.sensor_brand_id == pkId);
                (item as sensor_brand).sensor_brand_id = pkId;
                this.entityList[index] = item as sensor_brand;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_brand))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as sensor_brand); });
            }
