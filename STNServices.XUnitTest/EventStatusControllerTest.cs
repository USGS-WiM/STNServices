...
namespace STNServices.XUnitTest
{
    ...
    {
        public EventStatusController controller { get; private set; }
        public EventStatusTest() {
            //Arrange
            controller = new EventStatusController(new InMemoryEventStatusAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<event_status>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Completed", result.LastOrDefault().status);
        }
        ...
        public async Task Get()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<event_status>(okResult.Value);
        ...
        {
            //Arrange
            var entity = new event_status() { status = "TestPost" };
        ...
        public async Task Put()
        {
            ...
            var entity = Assert.IsType<event_status>(okgetResult.Value);
            

            var newEntity = new event_status();
            newEntity.status = "active";
            //should test the equals Equatable for all these too
            var huh = entity.Equals(newEntity);

            entity.status = "editStat";
            //Act
    ...
    public class InMemoryEventStatusAgent : ISTNServicesAgent
    {
        private List<event_status> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryEventStatusAgent() {
           this.entityList = new List<event_status>()
           {
               new event_status() { event_status_id = 1, status= "Active" },
               new event_status() { event_status_id = 2, status= "Completed"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(event_status))
        ...
        {
            if (typeof(T) == typeof(event_status))
                return Task.Run(()=> { return entityList.Find(i => i.event_status_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(event_status))
            {
                entityList.Add(item as event_status);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(event_status))
            {
                entityList.AddRange(items.Cast<event_status>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(event_status))
            {
                var index = this.entityList.FindIndex(x => x.event_status_id == pkId);
                (item as event_status).event_status_id = pkId;
                this.entityList[index] = item as event_status;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(event_status))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as event_status); });
            }
