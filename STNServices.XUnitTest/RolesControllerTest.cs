...
namespace STNServices.XUnitTest
{
    public class RolesTest
    {
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<roles>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Manager", result.LastOrDefault().role_name);
        }
        ...
        public async Task Get()
        {
            ...
            Assert.Equal("Admin", result.role_name);
        }
        ...
        {
            //Arrange
            var entity = new roles() { role_name = "TestPost" };
            ...
            Assert.Equal("TestPost", result.role_name);
        }
    ...
    public class InMemoryRolesAgent : ISTNServicesAgent
    {
        private List<roles> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryRolesAgent()
        {
            this.entityList = new List<roles>()
           {
               new roles() { role_id = 1, role_name= "Admin", role_description = "Administrator" },
               new roles() { role_id = 2, role_name= "Manager",role_description = "Manager (web page, provisional data)"  }
           };
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(roles))
        ...
        {
            if (typeof(T) == typeof(roles))
                return Task.Run(() => { return entityList.Find(i => i.role_id == pk) as T; });
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(roles))
            {
                entityList.AddRange(items.Cast<roles>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(roles))
            {
                var index = this.entityList.FindIndex(x => x.role_id == pkId);
                (item as roles).role_id = pkId;
                this.entityList[index] = item as roles;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(roles))
            {
                return Task.Run(() => { this.entityList.Remove(item as roles); });
            }
