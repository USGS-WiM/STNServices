...
namespace STNServices.XUnitTest
{
    ...
    {
        public InstrumentStatusController controller { get; private set; }
        public InstrumentStatusTest() {
            //Arrange
            controller = new InstrumentStatusController(new InMemoryInstrumentStatusAgent());
        ...
        {            
            ...
            var result = Assert.IsType<EnumerableQuery<instrument_status>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal(22, result.LastOrDefault().instrument_id);
        }
        ...
        public async Task Get()
        {
            ...
            Assert.Equal(11, result.instrument_id);
            Assert.Equal(1, result.status_type_id);
        }
        ...
        {
            //Arrange
            var entity = new instrument_status() { instrument_id = 113, status_type_id = 3, time_stamp = new DateTime(2017, 11, 03), time_zone = "UTC", member_id = 13 };
        ...
        {           
            ...
            var entity = Assert.IsType<instrument_status>(okgetResult.Value);
            
            entity.status_type_id = 3;
            entity.time_stamp = new DateTime(2017, 11, 22);
            //Act
            ...
            Assert.Equal(entity.status_type_id, result.status_type_id);
            Assert.Equal(entity.time_stamp, result.time_stamp);
        }
        ...
        { 
            ...
            Assert.Equal(22, result.LastOrDefault().instrument_id);
            Assert.Equal(2, result.LastOrDefault().status_type_id);
        }
    }

    public class InMemoryInstrumentStatusAgent : ISTNServicesAgent
    {
        private List<instrument_status> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryInstrumentStatusAgent() {
           this.entityList = new List<instrument_status>()
           {
               new instrument_status() { instrument_status_id = 1, instrument_id = 11, status_type_id = 1, time_stamp = new DateTime(2017, 11, 01), time_zone= "UTC", member_id = 1 },
               new instrument_status() { instrument_status_id = 2, instrument_id = 22, status_type_id= 2, time_stamp = new DateTime(2017, 11, 02), time_zone= "UTC", member_id = 21 }
           };
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(instrument_status))
        ...
        {
            if (typeof(T) == typeof(instrument_status))
                return Task.Run(()=> { return entityList.Find(i => i.instrument_status_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(instrument_status))
            {
                entityList.Add(item as instrument_status);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(instrument_status))
            {
                entityList.AddRange(items.Cast<instrument_status>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(instrument_status))
            {
                var index = this.entityList.FindIndex(x => x.instrument_status_id == pkId);
                (item as instrument_status).instrument_status_id = pkId;
                this.entityList[index] = item as instrument_status;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(instrument_status))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as instrument_status); });
            }
