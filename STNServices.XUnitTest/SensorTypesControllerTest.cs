...
namespace STNServices.XUnitTest
{
    ...
    {
        public SensorTypesController controller { get; private set; }
        public SensorTypesTest() {
            //Arrange
            controller = new SensorTypesController(new InMemorySensorTypesAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<sensor_type>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Thermometer", result.LastOrDefault().sensor);
        }

        [Fact]
        public async Task Get() //wont work here(sensor tpes with deployment types)
        {
            //Arrange
            var id = 1;
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<sensor_type>(okResult.Value);
            
            Assert.Equal("Pressure Transducer", result.sensor);
        }
        ...
        {
            //Arrange
            var entity = new sensor_type() { sensor = "TestPost" };
            ...
            Assert.Equal("TestPost", result.sensor);
        }
        ...
        public async Task Put()
        {
            ...
            var entity = Assert.IsType<sensor_type>(okgetResult.Value);
            

            var newEntity = new sensor_type();
            newEntity.sensor = "pressure Transducer";
            //should test the equals Equatable for all these too
            var huh = entity.Equals(newEntity);

            entity.sensor = "testEdit";
            //Act
            ...
            Assert.Equal(entity.sensor, result.sensor);
        }
        ...
        public async Task Delete()
        {
            ...
            var response = await controller.Get(); //wont work here (sensor tpes with deployment types)
    ...
    public class InMemorySensorTypesAgent : ISTNServicesAgent
    {
        private List<sensor_type> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemorySensorTypesAgent() {
           this.entityList = new List<sensor_type>()
           {
               new sensor_type() { sensor_type_id = 1, sensor= "Pressure Transducer" },
               new sensor_type() { sensor_type_id = 2, sensor= "Thermometer"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(sensor_type))
        ...
        {
            if (typeof(T) == typeof(sensor_type))
                return Task.Run(()=> { return entityList.Find(i => i.sensor_type_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_type))
            {
                entityList.Add(item as sensor_type);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_type))
            {
                entityList.AddRange(items.Cast<sensor_type>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_type))
            {
                var index = this.entityList.FindIndex(x => x.sensor_type_id == pkId);
                (item as sensor_type).sensor_type_id = pkId;
                this.entityList[index] = item as sensor_type;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(sensor_type))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as sensor_type); });
            }
