...
namespace STNServices.XUnitTest
{
    ...
    {
        public NetworkTypesController controller { get; private set; }
        public NetworkTypesTest() {
            //Arrange
            controller = new NetworkTypesController(new InMemoryNetworkTypesAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<network_type>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("SWaTH", result.LastOrDefault().network_type_name);
        }
        ...
        public async Task Get()
        {
            ...
            Assert.Equal("Not Defined", result.network_type_name);
        }
        ...
        {
            //Arrange
            var entity = new network_type() { network_type_name = "TestPost" };
    ...
    public class InMemoryNetworkTypesAgent : ISTNServicesAgent
    {
        private List<network_type> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryNetworkTypesAgent() {
           this.entityList = new List<network_type>()
           {
               new network_type() { network_type_id = 1, network_type_name= "Not Defined" },
               new network_type() { network_type_id = 2, network_type_name= "SWaTH"  }
           };                 
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(network_type))
        ...
        {
            if (typeof(T) == typeof(network_type))
                return Task.Run(()=> { return entityList.Find(i => i.network_type_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(network_type))
            {
                entityList.Add(item as network_type);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(network_type))
            {
                entityList.AddRange(items.Cast<network_type>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(network_type))
            {
                var index = this.entityList.FindIndex(x => x.network_type_id == pkId);
                (item as network_type).network_type_id = pkId;
                this.entityList[index] = item as network_type;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(network_type))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as network_type); });
            }
