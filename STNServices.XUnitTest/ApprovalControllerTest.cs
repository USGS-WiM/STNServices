...
namespace STNServices.XUnitTest
{
    ...
    {
        public ApprovalsController controller { get; private set; }
        public ApprovalTest() {
            //Arrange
            controller = new ApprovalsController(new InMemoryApprovalAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<approval>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal(new DateTime(2017, 12, 06), result.LastOrDefault().approval_date);
        ...
        public async Task Get()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<approval>(okResult.Value);
            
            Assert.Equal(new DateTime(2016, 12, 06), result.approval_date);
            Assert.Equal(1, result.member_id);
        }
        
    }

    public class InMemoryApprovalAgent : ISTNServicesAgent
    {
        private List<approval> entityList { get; set; }

        public List<Message> Messages { get; set; }// => throw new NotImplementedException();

        public InMemoryApprovalAgent()
        {
            this.entityList = new List<approval>()
           {
               new approval() { approval_id = 1, approval_date= new DateTime(2016, 12, 06), member_id = 1 },
               new approval() { approval_id = 2, approval_date= new DateTime(2017, 12, 06), member_id = 2  }
           };
        }
        ...
        {
            if (typeof(T) == typeof(approval))
                return Task.Run(()=> { return entityList.Find(i => i.approval_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(approval))
            {
                entityList.Add(item as approval);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(approval))
            {
                entityList.AddRange(items.Cast<approval>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(approval))
            {
                var index = this.entityList.FindIndex(x => x.approval_id == pkId);
                (item as approval).approval_id = pkId;
                this.entityList[index] = item as approval;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(approval))
            {                
                return Task.Run(()=> { this.entityList.Remove(item as approval); });
            }
