...
namespace STNServices.XUnitTest
{
    public class SourcesTest
    {
        ...
        {
            //Arrange
            controller = new SourcesController(new InMemorySourcesAgent());
        ...
        public async Task GetAll()
        {
            ...
            var result = Assert.IsType<EnumerableQuery<sources>>(okResult.Value);

            Assert.Equal(2, result.Count());
            Assert.Equal("Jack", result.LastOrDefault().source_name);
        }
        ...
        public async Task Get()
        {
            ...
            // Assert
            var okResult = Assert.IsType<OkObjectResult>(response);
            var result = Assert.IsType<sources>(okResult.Value);

            Assert.Equal("Tom", result.source_name);
        }
        ...
        {
            //Arrange
            var entity = new sources() { source_name = "Jill" };
            ...
            Assert.Equal("Jill", result.source_name);
        }
        ...
        public async Task Put()
        {
            ...
            var entity = Assert.IsType<sources>(okgetResult.Value);


            var newEntity = new sources();
            newEntity.source_name = "tom";
            //should test the equals Equatable for all these too
            var huh = entity.Equals(newEntity);

            entity.source_name = "testEdit";
            // Act
            ...
            Assert.Equal(entity.source_name, result.source_name);
        }

        
    }

    public class InMemorySourcesAgent : ISTNServicesAgent
    {
        private List<sources> entityList { get; set; }

        public List<Message> Messages { get; set; }

        public InMemorySourcesAgent()
        {
            this.entityList = new List<sources>()
           {
               new sources() { source_id = 1, source_name= "Tom", agency_id = 1 },
               new sources() { source_id = 2, source_name= "Jack", agency_id = 2  }
           };
        }

        public IQueryable<T> Select<T>() where T : class, new()
        {
            if (typeof(T) == typeof(sources))
        ...
        {
            if (typeof(T) == typeof(sources))
                return Task.Run(() => { return entityList.Find(i => i.source_id == pk) as T; });
        ...
        public Task<T> Add<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(sources))
            {
                entityList.Add(item as sources);
            }
        ...
        public Task<IEnumerable<T>> Add<T>(List<T> items) where T : class, new()
        {
            if (typeof(T) == typeof(sources))
            {
                entityList.AddRange(items.Cast<sources>());
            }
        ...
        public Task<T> Update<T>(int pkId, T item) where T : class, new()
        {
            if (typeof(T) == typeof(sources))
            {
                var index = this.entityList.FindIndex(x => x.source_id == pkId);
                (item as sources).source_id = pkId;
                this.entityList[index] = item as sources;
        ...
        public Task Delete<T>(T item) where T : class, new()
        {
            if (typeof(T) == typeof(sources))
            {
                return Task.Run(() => { this.entityList.Remove(item as sources); });
            }
