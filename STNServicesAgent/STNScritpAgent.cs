...
namespace STNAgent
{ 
    public class STNScriptAgent : ExternalProcessServiceAgentBase
{
    #region Properties
    ...
    STNServiceAgent sa = new STNServiceAgent(); 
    ...
    #region Constructors
    // for storm script
    public STNScriptAgent(Int32 pressureSensorDFID, Int32 waterSensorDFID, string username)
    ...
    // for air only script
    public STNScriptAgent(Int32 pressureSensorDFID, string username)
        : base(ConfigurationManager.AppSettings["EXEPath"], Path.Combine(new String[] { AppDomain.CurrentDomain.BaseDirectory, "Assets", "Scripts" }))
    {
        AirInit(pressureSensorDFID, username);
    }
    // for chopper script
    public STNScriptAgent(Int32 instrument_id, MemoryStream ms, string fileName) //uploadFile.instrument_id, memoryStream
        : base(ConfigurationManager.AppSettings["EXEPath"], Path.Combine(new String[] { AppDomain.CurrentDomain.BaseDirectory, "Assets", "Scripts" }))
    {
